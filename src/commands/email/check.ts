...
export default class EmailCheck extends BaseCommand {
	static override description = "Sync inbound emails from Resend";

	static override examples = [
		"<%= config.bin %> email:check",
		"<%= config.bin %> email:check --limit 100",
	];
	...
	async run(): Promise<void> {
		const { flags } = await this.parse(EmailCheck);
		...
		if (result.unmatched.length > 0) {
			...
			this.log("Tip: use `pipeline contact:add` to add these senders, then re-check.");
		}
	}
}
