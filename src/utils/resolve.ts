...
export async function resolveContactId(db: PipelineDB, query: string): Promise<{ id: number; name: string }> {
	const contacts = getContactsForFuzzy(db);
	return fuzzyResolve(contacts, query, "contact", ["name", "email"]);
}

export async function resolveDealId(db: PipelineDB, query: string): Promise<number> {
	const deals = getDealsForFuzzy(db);
	const match = await fuzzyResolve(deals, query, "deal");
	return match.id;
}

export async function resolveOrgId(db: PipelineDB, query: string): Promise<number> {
